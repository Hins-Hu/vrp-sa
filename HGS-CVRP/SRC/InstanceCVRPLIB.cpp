...
// Modified by Hins-Hu on 05/29/2023
//
...
{
	std::string content, content2, content3, content4;
	...
	if (inputFile.is_open())
	{
		...
		for (inputFile >> content ; content != "NODE_COORD_SECTION" ; inputFile >> content)
		{
			...
			else if (content == "EDGE_WEIGHT_TYPE")	inputFile >> content2 >> content4;
		...
		inputFile >> content;
		// if (content != "DIST_SECTION") throw std::string("Unexpected data in input file: " + content);
		for (int i = 0; i <= nbClients; i++)
		{
			inputFile >> node_number;

			for (int j = 0; j <= nbClients; j++){
				inputFile >> dist_mtx[i][j];
			}
		}

		// Calculating 2D Euclidean Distance
		// else if (content4 == "EUC_2D"){
		// for (int i = 0; i <= nbClients; i++)
		// {
		// 	for (int j = 0; j <= nbClients; j++)
		// 	{
		...
		// }
		// else{
		// 	throw std::string("The EDGE_WEIGHT_TYPE is not expected.");
		// }
	}
	else
